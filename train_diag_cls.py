...
argparser.add_argument('--s', type=int, default=3)
...
wandb.init(
    project="grokking-diag-cls-v2",
    entity=config['wandb_entity'],
    name=f"N{args.n_train}-D{args.d}-S{args.s}-L{args.L}-A{args.alpha}-WD{args.wd}",
    config=vars(args)
)
wandb.run.log_code(".")

train_data, test_data = gen_sparse_linear_classification(args.data_seed, args.n_train, args.n_test, args.d, args.s, 'cuda')
